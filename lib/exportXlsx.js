...
async function buildScheduleWorkbook(rows) {
  ...
  const fixedCols = [
    ...
    { header: 'Weight (days)', key: 'weight', width: 12 },
  ...
  for (const r of rows) {
    ...
    const rowData = {
      ...
      status: r.status || '', weight: r.is_leaf ? r.weight : '',
    ...
    if (r.is_leaf) row.getCell('weight').alignment = { vertical: 'middle', horizontal: 'right' };
