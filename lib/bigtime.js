...
async function getSession() {
  ...
  if (!res.ok) {
    ...
    throw new Error(`BigTime session exchange failed (${res.status}): ${text.slice(0, 500)}`);
  }
...
async function bigtimeGet(path) {
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    throw new Error(`BigTime API error (${res.status}) on ${path}: ${text.slice(0, 500)}`);
  }
  return res.json();
}
...
module.exports = { isConfigured, missingVars, getSession, bigtimeGet, findProjectByName, getTaskBudgetStatus, projectDisplayName };
