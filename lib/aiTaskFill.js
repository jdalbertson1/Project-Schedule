...
const TASK_FORM_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      status: { type: 'string', enum: ['', 'Not Started', 'In Progress', 'Complete'], description: 'Status only if explicitly stated or strongly implied; otherwise empty string so it auto-derives from percent complete. "Complete" always sets percent complete to 100 regardless of what else is inferred.' },
    ...
    required: ['topLevelWbs', 'existingSubtaskWbs', 'newSubtaskTitle', 'title', 'startDate', 'deadline', 'lead', 'status', 'pct', 'notes'],
