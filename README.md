...
- **Schedule** — the full WBS table with the 48-month Gantt view, phase color coding, and inline editing of leaf rows. The WBS and Task/Subtask columns stay frozen while you scroll right through the Gantt view. Parent rows roll up automatically (min start, max deadline, **weighted** average % of leaf tasks). Drag any row by its ⋮⋮ handle to reorder it or drop it onto another row to nest it underneath — hover the top/bottom edge of a row to drop as a sibling before/after it, or the middle to drop inside it as a child. WBS numbers, sort order, and rollup percentages all recompute automatically; a formerly-leaf task you drop something onto becomes a parent row on the spot.
